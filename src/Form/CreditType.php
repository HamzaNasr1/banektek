...
class CreditType extends AbstractType
{
    ...
    {
        $builder
        ->add('montant_echeance', null, [
            'attr' => [
                'class' => 'form-control',
                'placeholder' => 'montant echeance',
                'readonly' => 'readonly'
            ]
        ])
            ->add('montant', null, [
            ...
            ->add('id_compte', EntityType::class, [
                ...
                'choice_label' => function (Compte $compte) {
                    // Inclure l'âge du compte dans les options avec l'attribut data-age
                    return $compte->getIdUser()->getNom() . ' - ' . $compte->getRib();
                },
                'choice_attr' => function ($choice, $key, $value) {
                    // Ajouter l'attribut data-age à chaque option avec l'âge calculé à partir de la date de naissance
                    $dateNaissance = $choice->getIdUser()->getDob();
                    $age = $dateNaissance->diff(new \DateTime('now'))->y;
                    $revenu = $choice->getRevenu();

                    // Ajouter l'attribut data-solde à chaque option avec le solde du compte
                    $solde = $choice->getSolde();
