...
{
    #[Route('/export/pdf', name: 'export_compte_to_pdf', methods: ['GET'])]
    public function exportcomptesToPdf(CompteRepository $compteRepository): Response
    {
        ...
        $dompdf->loadHtml($this->renderView('pdff/index.html.twig', [
            'compte' => $compte,
        ]));
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="comptes.pdf"');

        return $response;
    }
    #[Route('/', name: 'app_transaction_index', methods: ['GET'])]
   
    public function index(transactionRepository $transactionRepository,Request $request,EntityManagerInterface $entityManager,SessionInterface $session): Response
    ...
    {   if($session->get('id_agent')){
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if($transaction->getType() == "Retrait"){
               ...
               // $twilio->sendSMSTransaction($transaction->getMontant(),1);
            ...
            if($transaction->getType() == "Versement"){
               ...
               // $twilio->sendSMSTransaction($transaction->getMontant(),1);
                  ...
                  // $transaction->getIdCompte()->setSolde($transaction->getIdCompte()->getSolde() + $transaction->getMontant());
                  // $twilio->sendSMSTransaction($transaction->getMontant(),0);
